...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const body = interaction.options.getInteger('body') ?? 1;
 ...
 if(player.Dead){
      ...
      await utils.checkGameState(game.GAMESTATES, playerClass.Class_Name == "Clockwatcher");
        ...
        try {
                ...
                switch(stat) {//Update Stat
                    case "Health_Points":
                        if(body == 1){
                            ...
                            await models.Players.update({Health_Points2: player.Health_Points2 + amount}, {where: {Player_ID: player.Player_ID}}); 
                            if(hpBuyIndex == 3) await models.Players.update({HP_COST: 10}, {where: {Player_ID: player.Player_ID}});
                            else await models.Players.update({HP_COST: rangeAndHpCostArray[hpBuyIndex + 1]}, {where: {Player_ID: player.Player_ID}});
                            break;
                        }
                    case "Range_":
                        ...
                        if(rangeBuyIndex == 3) await models.Players.update({RANGE_COST: 10}, {where: {Player_ID: player.Player_ID}});
                        else await models.Players.update({RANGE_COST: rangeAndHpCostArray[rangeBuyIndex + 1]}, {where: {Player_ID: player.Player_ID}});
                        break;
                        }
                        else {
                            await models.Players.update({Range2: player.Range2 + amount}, {where: {Player_ID: player.Player_ID}}); 
                            if(rangeBuyIndex == 3) await models.Players.update({RANGE_COST: 10}, {where: {Player_ID: player.Player_ID}});
                            else await models.Players.update({RANGE_COST: rangeAndHpCostArray[rangeBuyIndex + 1]}, {where: {Player_ID: player.Player_ID}});
                            break;
                        }
                    case "Damage":
                        if(body == 1){
                            ...
                            await models.Players.update({Damage2: player.Damage2 + amount}, {where: {Player_ID: player.Player_ID}}); 
                            if(damageBuyIndex == 2) await models.Players.update({DAMAGE_COST: 16}, {where: {Player_ID: player.Player_ID}});
                            else await models.Players.update({DAMAGE_COST: damageCostArray[damageBuyIndex + 1]}, {where: {Player_ID: player.Player_ID}});
                            break;
                        }
	        ...
	        await interaction.editReply({ content: 'Confirmation not received within 1 minute, cancelling...', ephemeral: true });
        }
    }
    catch (error) {
        console.error(error);
