 ...
 * - the success message names the tile's PRE-burn type (the row was read
 *   before the update), so players see "made a Blank1 tile", never "Fire"
 * - occupied tiles, Fire tiles and every other non-gateway type still burn;
 *   only Gateway_Open/Gateway_Locked are blocked
 * - rejection order: missing tile, dead player, gamestate, class, gateway,
 *   range, AP
 */
...
async function run(input, deps = defaultDeps) {
  ...
  if (!tileToChange) {
    return { ok: false, reason: REJECTIONS.NO_SUCH_TILE, data: { action: 'burn' } };
  }
  ...
  if (tileToChange.Tile_Type === 'Gateway_Open' || tileToChange.Tile_Type === 'Gateway_Locked') {
    return { ok: false, reason: REJECTIONS.WRONG_TILE_TYPE, data: { message: 'You cannot burn a gateway tile!' } };
  }

  if (!tileInRange) {
    return { ok: false, reason: REJECTIONS.OUT_OF_RANGE, data: { message: 'You are not in range of the tile you want to burn!' } };
  }

  if (player.Action_Points < 4) {
    return { ok: false, reason: REJECTIONS.NOT_ENOUGH_AP, data: { action: 'burn a tile' } };
  }
  ...
  return {
    ...
    data: {
      x: input.x,
      y: input.y,
      // the pre-burn type, as the old message showed (the row was read
      // before the update)
