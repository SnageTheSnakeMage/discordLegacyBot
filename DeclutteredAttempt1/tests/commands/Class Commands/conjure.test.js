/**
 * /conjure - logic tests. Plain data in, plain data out: no jest.mock of
 ...
 * Player: Druid at (1,1), Range_ 3, 5 AP. Target tile: Blank1 at (2,1),
...
function happyDeps(over = {}) {
  ...
  const player = over.player || createFakePlayer({
    Player_ID: 1, Discord_ID: DRUID, Action_Points: 5, Range_: 3, Tile_ID: 1, Class_ID: 7,
  });
  const playerClass = over.playerClass || createFakeClass({ Class_ID: 7, Class_Name: 'Druid' });
...
const INPUT = { x: 2, y: 1, gameId: 1, discordId: DRUID, username: 'snage' };
...
describe('conjure.parse', () => {
  it('maps raw options and applies defaults', () => {
    const input = logic.parse({ x: 4, y: 2, game: null }, { discordId: DRUID, username: 'snage' });
    expect(input).toEqual({ x: 4, y: 2, gameId: null, discordId: DRUID, username: 'snage' });
  });

  it('keeps an explicit game id', () => {
    const input = logic.parse({ x: 1, y: 1, game: 3 }, { discordId: DRUID, username: 'snage' });
    expect(input.gameId).toBe(3);
  });
});

describe('conjure.run rejections', () => {
  ...
  it('rejects a dead player', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: DRUID, Action_Points: 5, Range_: 3, Tile_ID: 1, Dead: true }),
    });
    const result = await logic.run(INPUT, deps);
  ...
  // (a Druid is never a Clockwatcher) - preserved.
  it.each([
  ...
  it('rejects when there is no tile at the coordinates', async () => {
    ...
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.NO_SUCH_TILE, data: { action: 'conjure a storm on' } });
    expectNoWrites(deps);
  });

  it('rejects a non-Druid', async () => {
    const { deps } = happyDeps({ playerClass: createFakeClass({ Class_Name: 'Average' }) });
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.WRONG_CLASS, data: { className: 'Druid' } });
    expectNoWrites(deps);
  });
  ...
  it('rejects with 3 AP, one short of the 4 AP cost (boundary: one short)', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: DRUID, Action_Points: 3, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.NOT_ENOUGH_AP, data: { action: 'conjure a storm on a tile' } });
    expectNoWrites(deps);
  });

  it('accepts with exactly 4 AP (boundary: exact)', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: DRUID, Action_Points: 4, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
    expect(result.ok).toBe(true);
...
describe('conjure.run success', () => {
  it('conjures the storm with exact write payloads', async () => {
    const { deps } = happyDeps();
    const result = await logic.run(INPUT, deps);
  ...
  it('re-conjures a tile that is already Storm (preserved quirk)', async () => {
    ...
    const result = await logic.run(INPUT, deps);
    expect(result.ok).toBe(true);
    expect(result.data.previousTileType).toBe('Storm');
  });
  ...
  it('resolves the default game via getOldestActiveGameId when no game is given', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(deps.utils.getOldestActiveGameId).toHaveBeenCalledWith(DRUID);
  });
});
