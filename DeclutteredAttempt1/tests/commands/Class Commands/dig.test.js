/**
 * /dig - logic tests. Plain data in, plain data out: no jest.mock of
 ...
 * Player: Gravedigger at (1,1), Range_ 3, 5 AP. Target tile: empty Blank1
 * at (2,1), Tile_ID 42, same layer. Range semantics: getTileCordinatesOfLine
 * includes the start tile, so (1,1)->(3,1) is length 3 (in range at Range_ 3)
 * and (1,1)->(4,1) is length 4 (one beyond).
 */
function happyDeps(over = {}) {
  ...
  const player = over.player || createFakePlayer({
    Player_ID: 1, Discord_ID: DIGGER, Action_Points: 5, Range_: 3, Tile_ID: 1, Class_ID: 7,
  });
  const playerClass = over.playerClass || createFakeClass({ Class_ID: 7, Class_Name: 'Gravedigger' });
...
describe('dig.parse', () => {
  it('maps raw options and applies defaults', () => {
    const input = logic.parse({ x: 4, y: 2, game: null }, { discordId: DIGGER, username: 'snage' });
    expect(input).toEqual({ x: 4, y: 2, gameId: null, discordId: DIGGER, username: 'snage' });
  });

  it('keeps an explicit game id', () => {
    const input = logic.parse({ x: 1, y: 1, game: 3 }, { discordId: DIGGER, username: 'snage' });
    expect(input.gameId).toBe(3);
  });
});

describe('dig.run rejections', () => {
  ...
  it('rejects a player who is not in the game (looked up by Discord_ID, the old Player_ID lookup was a crash)', async () => {
    ...
    expect(deps.models.Players.findOne).toHaveBeenCalledWith(
      { where: { Game_ID: 1, Discord_ID: DIGGER } },
    );
    expectNoWrites(deps);
  });
  ...
  it('rejects when there is no tile at the coordinates', async () => {
    ...
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.NO_SUCH_TILE, data: { action: 'dig' } });
    expectNoWrites(deps);
  });

  it('rejects a non-Gravedigger', async () => {
    const { deps } = happyDeps({ playerClass: createFakeClass({ Class_Name: 'Average' }) });
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.WRONG_CLASS, data: { className: 'Gravedigger' } });
    expectNoWrites(deps);
  });
  ...
  it('rejects with 3 AP, one short of the 4 AP cost (boundary: one short)', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: DIGGER, Action_Points: 3, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.NOT_ENOUGH_AP, data: { action: 'dig a tile' } });
    expectNoWrites(deps);
  });

  it('accepts with exactly 4 AP (boundary: exact)', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: DIGGER, Action_Points: 4, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
    expect(result.ok).toBe(true);
...
describe('dig.run success', () => {
  it('digs the tile with exact write payloads', async () => {
    const { deps } = happyDeps();
    const result = await logic.run(INPUT, deps);
  ...
  it('re-digs a tile that is already Void (preserved quirk)', async () => {
    ...
    const result = await logic.run(INPUT, deps);
    expect(result.ok).toBe(true);
    expect(result.data.previousTileType).toBe('Void');
  });
  ...
  it('resolves the default game via getOldestGameId when no game is given', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(deps.utils.getOldestGameId).toHaveBeenCalledWith(DIGGER);
  });
});
