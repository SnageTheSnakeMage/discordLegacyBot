/**
 * /hide - logic tests. Plain data in, plain data out: no jest.mock of
 ...
 * Player: Hunter at (1,1), Range_ 3, 5 AP. Target tile: Blank1 at (2,1),
...
function happyDeps(over = {}) {
  ...
  const player = over.player || createFakePlayer({
    Player_ID: 1, Discord_ID: HUNTER, Action_Points: 5, Range_: 3, Tile_ID: 1, Class_ID: 9,
  });
  const playerClass = over.playerClass || createFakeClass({ Class_ID: 9, Class_Name: 'Hunter' });
...
const INPUT = { x: 2, y: 1, gameId: 1, discordId: HUNTER, username: 'snage' };
...
describe('hide.parse', () => {
  it('maps raw options and applies defaults', () => {
    const input = logic.parse({ x: 4, y: 2, game: null }, { discordId: HUNTER, username: 'snage' });
    expect(input).toEqual({ x: 4, y: 2, gameId: null, discordId: HUNTER, username: 'snage' });
  });

  it('keeps an explicit game id', () => {
    const input = logic.parse({ x: 1, y: 1, game: 3 }, { discordId: HUNTER, username: 'snage' });
    expect(input.gameId).toBe(3);
  });

  it('turns an absent (undefined) game option into null', () => {
    const input = logic.parse({ x: 1, y: 1 }, { discordId: HUNTER, username: 'snage' });
    expect(input.gameId).toBeNull();
  });
});

describe('hide.run rejections', () => {
  ...
  // (a Hunter is never a Clockwatcher) - preserved.
  it.each([
  ...
  it('rejects when there is no tile at the coordinates', async () => {
    ...
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.NO_SUCH_TILE, data: { action: 'hide' } });
    expectNoWrites(deps);
  });
  ...
  it('rejects a non-Hunter', async () => {
    const { deps } = happyDeps({ playerClass: createFakeClass({ Class_Name: 'Average' }) });
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.WRONG_CLASS, data: { className: 'Hunter' } });
    expectNoWrites(deps);
  });
  ...
  it('rejects with 4 AP, one short of the 5 AP cost (boundary: one short)', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: HUNTER, Action_Points: 4, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.NOT_ENOUGH_AP, data: { action: 'hide a tile' } });
    expectNoWrites(deps);
  });

  it('accepts with exactly 5 AP (boundary: exact)', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: HUNTER, Action_Points: 5, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
    expect(result.ok).toBe(true);
...
describe('hide.run success', () => {
  it('hides the tile with exact write payloads', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: HUNTER, Action_Points: 7, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
  ...
  it('resolves the default game via getOldestGameId when no game is given', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(deps.utils.getOldestGameId).toHaveBeenCalledWith(HUNTER);
  });
});
