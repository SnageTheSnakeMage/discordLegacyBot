/**
 * /heal - logic tests. Plain data in, plain data out: no jest.mock of
 ...
 * Player: Doctor at (1,1), Range_ 3, 5 AP. Target tile: Blank1 at (2,1),
...
function happyDeps(over = {}) {
  ...
  const player = over.player || createFakePlayer({
    Player_ID: 1, Discord_ID: DOCTOR, Action_Points: 5, Range_: 3, Tile_ID: 1, Class_ID: 9,
  });
  const playerClass = over.playerClass || createFakeClass({ Class_ID: 9, Class_Name: 'Doctor' });
...
const INPUT = { x: 2, y: 1, gameId: 1, discordId: DOCTOR };
...
describe('heal.parse', () => {
  it('maps raw options and applies defaults', () => {
    const input = logic.parse({ x: 4, y: 2, game: null }, { discordId: DOCTOR, username: 'snage' });
    expect(input).toEqual({ x: 4, y: 2, gameId: null, discordId: DOCTOR });
  });

  it('keeps an explicit game id', () => {
    const input = logic.parse({ x: 1, y: 1, game: 3 }, { discordId: DOCTOR, username: 'snage' });
    expect(input.gameId).toBe(3);
  });

  it('turns an absent game option into null', () => {
    const input = logic.parse({ x: 1, y: 1, game: undefined }, { discordId: DOCTOR, username: 'snage' });
    expect(input.gameId).toBeNull();
  });
});

describe('heal.run rejections', () => {
  ...
  it('rejects when there is no tile at the coordinates', async () => {
    ...
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.NO_SUCH_TILE, data: { action: 'heal' } });
    expectNoWrites(deps);
  });
  ...
  it('blocks a Clockwatcher during a timestop too (isClockwatcher is hard-coded false - preserved quirk)', async () => {
    const { deps } = happyDeps({
  ...
  it('reports the gamestate before the missing tile (legacy order)', async () => {
    ...
    const result = await logic.run(INPUT, deps);
    expect(result.reason).toBe(REJECTIONS.GAME_PAUSED);
  });

  it('rejects a non-Doctor', async () => {
    const { deps } = happyDeps({ playerClass: createFakeClass({ Class_Name: 'Average' }) });
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.WRONG_CLASS, data: { className: 'Doctor' } });
    expectNoWrites(deps);
  });
  ...
  it('rejects with 4 AP, one short of the 5 AP cost (boundary: one short)', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: DOCTOR, Action_Points: 4, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
    expect(result).toMatchObject({ ok: false, reason: REJECTIONS.NOT_ENOUGH_AP, data: { action: 'heal a tile' } });
    expectNoWrites(deps);
  });

  it('accepts with exactly 5 AP (boundary: exact)', async () => {
    const { deps } = happyDeps({
      player: createFakePlayer({ Player_ID: 1, Discord_ID: DOCTOR, Action_Points: 5, Range_: 3, Tile_ID: 1 }),
    });
    const result = await logic.run(INPUT, deps);
    expect(result.ok).toBe(true);
...
describe('heal.run success', () => {
  it('heals the tile with exact write payloads', async () => {
    const { deps } = happyDeps();
    const result = await logic.run(INPUT, deps);
  ...
  it('finds the player by the resolved game id and discord id', async () => {
    ...
    expect(deps.models.Players.findOne).toHaveBeenCalledWith(
      { where: { Game_ID: 1, Discord_ID: DOCTOR } },
    );
  });
  ...
  it('resolves the default game via getOldestGameId, passing the actor id (the old call passed nothing and threw)', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(deps.utils.getOldestGameId).toHaveBeenCalledWith(DOCTOR);
  });
});
...
describe('heal.present', () => {
  ...
  it('renders the unknown-game rejection with the game id', () => {
    expect(logic.present({ ok: false, reason: REJECTIONS.NO_SUCH_GAME, data: { gameId: 7 } }))
